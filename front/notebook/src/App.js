...
function App() {
  ...
  const apiUrl = '//localhost:3000'; // قم بتغيير عنوان الخادم إلى عنوان الخادم الفعلي الذي تستخدمه
...
const deleteNoteFromServer = (note) => {
  ...
  useEffect(() => {
    // استدعاء fetchData عند التحميل الأولي
    fetchData();

    // ضبط تحديث البيانات كل 500 ميلي ثانية
    const intervalId = setInterval(fetchData, 500);

    // إلغاء تأمين الفاصل الزمني عندما يتم إلغاء تحميل المكون
    return () => clearInterval(intervalId);
  }, []); // [] يضمن أن useEffect يستدعى مرة واحدة عند التحميل الأولي

  // الكود الباقي لعرض البيانات أو أي شيء آخر
